...
--1. Select all records in table “Employees” whose last name starts with ‘Sah’
    ...
    SELECT last_name FROM employees WHERE last_name LIKE '%sah';
...
-- 2022. Start retrieving from the 4th record in the table
    ...
    SELECT employee_id,first_name,last_name FROM employees WHERE hire_date >= '2021-06-01' and hire_date <= '2022-02-15';

-- 3. Find distinct top 5 salary values (only salary column), using alias for column as
-- “Top 5

    SELECT DISTINCT salary as Top_5 FROM employees ORDER BY salary LIMIT 5;
